...
def create_scenario(world):
    ...
    # Conexões val_out/val_in REMOVIDAS — mensagens entre agentes agora
    # trafegam diretamente pelo PADE via self.agent.send() (FIPA nativo).
    # O Mosaik não é mais o canal de transporte das mensagens ACL.

    for i in range(1, NUM_PERIFERICOS + 1):
        pade_sim.PadeAgent(agent_id=f'AgenteP_{i}')

    # Conexão de telemetria mantida — o coletor ainda lê dados da rede OMNeT++
    '''world.connect(rede_omnet, monitor,
                  'status', 'packets_sent', 'packets_received', 'packets_dropped',
                  'packet_sizes_out', 'latencies_out', 'jitters_out', 'val_out')''' #as mensagens agora trafegam por fora (via ZMQ) e o omnet não vai mais entregar pacotes de dados para o Mosaik, ele age agora apenas como orquestrador e não mais ponto de troca de mensagens
    world.connect(rede_omnet, monitor, 
                    'status', 'packets_sent', 'packets_received', 'packets_dropped', 
